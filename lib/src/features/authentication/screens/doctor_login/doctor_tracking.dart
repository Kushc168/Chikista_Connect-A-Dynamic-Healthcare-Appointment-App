...
class _DoctorAppointmentsScreenState extends State<DoctorAppointmentsScreen> {
  ...
  void _nextAppointment() {
    setState(() {
      ...
      // Notify the user's side
      _notifyUser();
    });
  }

  void _notifyUser() {
    // Notify user's side here
    // You can use any method to notify the user about the updated appointment status
    print('Notify user: Appointment ${_currentAppointmentIndex + 1} Check-up Done');
  }
...
class LiveTrackingWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        children: [
          for (int i = 0; i < 11; i++) // 11 appointments
            Expanded(
              child: Container(
